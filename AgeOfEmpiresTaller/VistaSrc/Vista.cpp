...
bool Vista::loadMedia() {
	...
	for (it = modelo->juego->tipos.begin(); it != modelo->juego->tipos.end();
		...
		if (tipo->fps == 0) {
			...
			this->factory->crear_dibujo_personaje(tipo->imagen, MOVIMIENTOS,
			CANTIDAD_DE_IMAGENES, tipo->fps); //el ultimo parametro es velocidad
		}
...
void Vista::crearPersonaje(string tipo, Personaje* personaje) {
	...
	this->factory->crear_dibujo_personaje(obj->imagen, MOVIMIENTOS,
	CANTIDAD_DE_IMAGENES, obj->fps); //el ultimo parametro es velocidad
	dibujo_t pers_moviendo = this->factory->ultimo_dibujo();
	this->factory->crear_dibujo_personaje(obj->imagen_quieto, MOVIMIENTOS,
	CANTIDAD_DE_IMAGENES, obj->fps); //el ultimo parametro es velocidad
	dibujo_t pers_esta_quieto = this->factory->ultimo_dibujo();
	this->factory->crear_dibujo_personaje(obj->imagen_atacando, MOVIMIENTOS,
	CANTIDAD_DE_IMAGENES, obj->fps); //el ultimo parametro es velocidad
