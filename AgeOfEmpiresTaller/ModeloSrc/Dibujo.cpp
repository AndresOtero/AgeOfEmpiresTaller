...
void  Dibujo::render( int x, int y,SDL_Renderer* renderer ){
	this->textura->render(x,y,&(this->spriteClips[this->imagen_actual]),renderer);
	this->x_imagen=x;
	this->y_imagen=y;
}
...
void Dibujo::mover(int x, int y) {
	...
	if (distancia != 0) {
		...
		y_imagen += des_y;
		imagen_actual++;
		if(imagen_actual>=cantidad_de_imagenes){
			imagen_actual=0;
		}
	}
}
